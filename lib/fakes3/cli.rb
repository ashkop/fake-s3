...
module FakeS3
  class CLI < Thor
    ...
    def server
      ...
      server = FakeS3::Server.new(address,options[:port],store,hostname,ssl_cert_path,ssl_key_path, quiet: !!options[:quiet], do_not_reverse_lookup: options[:do_not_reverse_lookup])
      server.serve
    end
